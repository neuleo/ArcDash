...
class SpeedCurveEditor extends StatefulWidget {
  final List<int> ratios; // 18 points: 500..9000 RPM (in 500 RPM steps)
...
class _SpeedCurveEditorState extends State<SpeedCurveEditor> {
  ...
  Widget build(BuildContext context) {
    final values =
        widget.ratios.length == 18 ? widget.ratios : List<int>.filled(18, 100);
      ...
      children: [
        Row(
          ...
          children: [
            Row(
              children: const [
                Icon(Icons.auto_graph, color: Color(0xFF00E5FF), size: 18),
                SizedBox(width: 8),
                Text(
                  'DREHZAHL-LEISTUNGSKURVE (500–9000 RPM)',
                  style: TextStyle(
            ...
            Container(
              ...
              child: Text(
                '${SpeedCurveEditor.rpmSteps[_selectedPointIndex]} RPM: ${values[_selectedPointIndex]}%',
          ...
          children: [
            _PresetCurveButton(
              ...
              onPressed: widget.enabled
                  ? () {
                      HapticFeedback.selectionClick();
                      widget.onChanged(List<int>.filled(18, 100));
                    }
                  : null,
            ),
            _PresetCurveButton(
              ...
              onPressed: widget.enabled
                  ? () {
                      HapticFeedback.selectionClick();
                      final list = List<int>.generate(18,
                          (i) => (40 + (i * 60 / 17)).round().clamp(0, 100));
              ...
              onPressed: widget.enabled
                  ? () {
                      ...
                      final list = List<int>.generate(18, (i) {
                        if (i < 12) return 100;
                        return (100 - (i - 11) * 10).clamp(30, 100);
                      });
                      widget.onChanged(list);
                    }
                  : null,
        ...
        Row(
          children: [
            SizedBox(
              width: 90,
              child: Text(
                '${SpeedCurveEditor.rpmSteps[_selectedPointIndex]} RPM',
                style: const TextStyle(
...
class _PresetCurveButton extends StatelessWidget {
  final String label;
  final VoidCallback? onPressed;

  const _PresetCurveButton({
    required this.label,
